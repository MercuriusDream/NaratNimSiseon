...
try:
    ...
    if hasattr(settings, 'GEMINI_API_KEY') and settings.GEMINI_API_KEY:
        ...
        model = genai.GenerativeModel('gemma-3-27b-it') # Main model for detailed analysis
    else:
        logger.warning("GEMINI_API_KEY not found or empty in settings. LLM features will be disabled.")
        genai = None
        model = None
except ImportError:
    logger.warning("google.generativeai library not available. LLM features will be disabled.")
    genai = None
    model = None
except Exception as e:
    logger.warning(f"Error configuring Gemini API: {e}. LLM features will be disabled.")
    genai = None
    model = None
...
def is_celery_available():
    ...
    try:
        ...
        return True
    except (ImportError, OperationalError, OSError, ConnectionError, AttributeError) as e:
        logger.warning(f"Celery not available or broker connection failed: {e}")
        return False
...
def celery_or_sync(func):
    ...
    def wrapper(*args, **kwargs):
        if is_celery_available():
            ...
            if hasattr(func, '__wrapped__') and 'bind' in func.__dict__.get('__header__', {}):
                 # Call the original function without 'self' if it's a bound task being run synchronously
                return func.__wrapped__(None, *args, **kwargs) # Pass None for self
...
def fetch_speaker_details(speaker_name):
    ...
    try:
        if not hasattr(settings, 'ASSEMBLY_API_KEY') or not settings.ASSEMBLY_API_KEY:
            logger.error("ASSEMBLY_API_KEY not configured for fetch_speaker_details.")
            return None
        ...
        params = {
            ...
            "Type": "json",
            "pSize": 5 # Fetch a few in case of name ambiguity, pick the best match
        }
        ...
        if not member_data_list:
            logger.warning(f"⚠️ No member data found for: {speaker_name} via ALLNAMEMBER API.")
            return None
        ...
        speaker, created = Speaker.objects.update_or_create(
            ...
            defaults={
                ...
                'gtelt_eraco': member_data.get('GTELT_ERACO', ''), # Era might be important
...
def fetch_continuous_sessions(self, # Celery provides 'self'
                              force=False,
                              debug=False,
                              start_date=None):
    ...
    try:
        ...
        for months_back in range(0, 36):
            target_date = current_date - timedelta(days=months_back * 30.44) # Approximate month step back
            ...
            try:
                ...
                if debug: logger.exception("Full traceback for error during loop:")
            continue
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def fetch_latest_sessions(self, force=False, debug=False): # Celery provides 'self'
    ...
    try:
        if not hasattr(settings, 'ASSEMBLY_API_KEY') or not settings.ASSEMBLY_API_KEY:
            ...
            raise ValueError("ASSEMBLY_API_KEY not configured") # Stop if key missing

        url = "https://open.assembly.go.kr/portal/openapi/nzbyfwhwaoanttzje"
        DAE_NUM_TARGET = "22" # Make configurable if needed

        if not force:
            logger.info("📅 Fetching sessions for current and previous month (non-force mode).")
            dates_to_check = [
            ...
            for conf_date_str in unique_conf_dates:
                ...
                logger.info(f"📅 Fetching sessions for: {conf_date_str}")
                if debug: logger.debug(f"🐛 DEBUG: API URL: {url}, Params: {params}")
                ...
                try:
                    ...
                    sessions_data = extract_sessions_from_response(data, debug=debug)
                    if sessions_data:
                        process_sessions_data(sessions_data, force=False, debug=debug) # force is False here
                    else:
                        logger.info(f"No sessions data found for {conf_date_str} in non-force mode.")
                    if not debug: time.sleep(1)
                except requests.exceptions.RequestException as e:
                    logger.warning(f"⚠️ Request error fetching {conf_date_str} (non-force): {e}")
                except json.JSONDecodeError as e:
                    logger.warning(f"⚠️ JSON parsing error for {conf_date_str} (non-force): {e}")
                except Exception as e:
                    logger.warning(f"⚠️ Error processing {conf_date_str} (non-force): {e}")
        else: # Force mode
            logger.info("🔄 Force mode: Fetching sessions month by month for up to 24 months.")
            current_loop_date = datetime.now()
            for months_back in range(0, 24):
                target_date = current_loop_date - timedelta(days=months_back * 30.44)
                conf_date_str = target_date.strftime('%Y-%m')
                params = {
                    "KEY": settings.ASSEMBLY_API_KEY, "Type": "json", "DAE_NUM": DAE_NUM_TARGET,
                    "CONF_DATE": conf_date_str, "pSize": 500
                }
                logger.info(f"📅 Fetching sessions for: {conf_date_str} (force mode)")
                if debug: logger.debug(f"🐛 DEBUG: API URL: {url}, Params for {conf_date_str}: {params}")
                ...
                try:
                    ...
                    sessions_data = extract_sessions_from_response(data, debug=debug)
                    if sessions_data:
                        process_sessions_data(sessions_data, force=True, debug=debug) # force is True here
                    else:
                        logger.info(f"❌ No sessions found for {conf_date_str} in force mode. Might be end of data for DAE_NUM {DAE_NUM_TARGET}.")
                        # Optionally break if no sessions found for a few consecutive months
                    if not debug: time.sleep(1)
                except requests.exceptions.RequestException as e:
                    logger.warning(f"⚠️ Request error fetching {conf_date_str} (force): {e}")
                except json.JSONDecodeError as e:
                    logger.warning(f"⚠️ JSON parsing error for {conf_date_str} (force): {e}")
                except Exception as e:
                    logger.warning(f"⚠️ Error processing {conf_date_str} (force): {e}")
                    if debug: logger.debug(f"🐛 DEBUG: Full error: {type(e).__name__}: {e}")
                continue

        if not debug: # Consider if this should run even if no sessions were found/updated
            logger.info("🔄 Triggering additional data collection after session fetch.")
            if is_celery_available():
                fetch_additional_data_nepjpxkkabqiqpbvk.delay(force=force, debug=debug)
            else:
                fetch_additional_data_nepjpxkkabqiqpbvk(force=force, debug=debug)
...
def extract_sessions_from_response(data, debug=False):
    ...
    if data and api_key_name in data and isinstance(data[api_key_name], list):
        if len(data[api_key_name]) > 1 and isinstance(data[api_key_name][1], dict):
            ...
            if debug: logger.debug(f"Extracted data from data['{api_key_name}'][1]['row']")
            ...
            if head_info:
                 result_code_info = head_info[0].get('RESULT', {}).get('CODE', 'UNKNOWN')
                 if result_code_info.startswith("INFO-") or result_code_info.startswith("ERROR-"): # Assuming "INFO-200" is no data
                     logger.info(f"API result indicates no data or error: {result_code_info} in head.")
                     ...
                     if 'row' in data[api_key_name][0]:
                         ...
                         if debug and sessions_data_list: logger.debug("Extracted data from data['{api_key_name}'][0]['row'] despite head info code.")


        if not sessions_data_list and debug: # if still no data and debug
             logger.debug(f"No 'row' found in expected paths data['{api_key_name}'][1] or data['{api_key_name}'][0]. API structure might have changed or no data.")
             # logger.debug(f"Full response for {api_key_name} if empty: {data[api_key_name]}")


    elif data and 'row' in data and isinstance(data['row'], list): # Fallback for simpler structure
        sessions_data_list = data['row']
        if debug: logger.debug("Extracted data from data['row'] (fallback).")
    else:
        if debug:
            logger.debug(f"Could not find session data in expected structures. Keys: {list(data.keys()) if data else 'Empty data'}")

    if debug and sessions_data_list:
        logger.debug(f"Extracted {len(sessions_data_list)} session items. Sample: {sessions_data_list[0] if sessions_data_list else 'None'}")
...
def process_sessions_data(sessions_data, force=False, debug=False):
    ...
    for item_data in sessions_data:
        ...
        if not confer_num:
            logger.warning(f"Skipping item due to missing CONFER_NUM: {item_data.get('TITLE', 'N/A')}")
            continue
    ...
    for confer_num, items_for_session in sessions_by_confer_num.items():
        ...
        try:
            ...
            logger.info(f"Processing session ID {confer_num}: {session_title} ({len(items_for_session)} items)")
            ...
            if conf_date_str:
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not parse date: {conf_date_str} for session {confer_num}. Skipping date.")
            ...
            if len(title_parts) > 1 and "회국회" in title_parts[1]: # e.g. 제400회국회(정기회)
                sess_val = title_parts[1].split('회국회')[0] # 400
                if "(" in sess_val: sess_val = sess_val.split("(")[0] # clean (정기회)
            ...
            session_defaults = {
                ...
                'sess': sess_val,
                'dgr': dgr_val,
                'conf_dt': conf_date_val,
                'conf_knd': main_item.get('CLASS_NAME', '국회본회의'), # Or COMMITTEE_NAME
                'cmit_nm': main_item.get('CMIT_NAME', main_item.get('CLASS_NAME', '국회본회의')), # Prefer CMIT_NAME if available
                'down_url': main_item.get('PDF_LINK_URL', ''),
                'title': session_title, # Store the full title
                 # bg_ptm might come from VCONFDETAIL, initialize if not set
                'bg_ptm': dt_time(9, 0) # Default, can be updated later
            }

            if debug:
                logger.debug(f"🐛 DEBUG: Session {confer_num} defaults for DB: {session_defaults}")
                logger.info(f"🐛 DEBUG PREVIEW: Would process session ID {confer_num}: {session_title}")
            ...
            if created:
                created_count += 1
                logger.info(f"✨ Created new session: {confer_num} - {session_title}")
            else:
                if force: # If not created but force is true, it implies update_or_create updated it.
                    updated_count += 1
                    logger.info(f"🔄 Updated existing session: {confer_num} - {session_title}")
                else:
                    logger.info(f"♻️ Session already exists and not in force mode: {confer_num}")


            # Regardless of created/updated, try to fetch more details
            # Call for details which then calls for bills and PDF
            if is_celery_available():
                fetch_session_details.delay(session_id=confer_num, force=force, debug=debug)
            else:
                fetch_session_details(session_id=confer_num, force=force, debug=debug)

        except Exception as e:
            logger.error(f"❌ Error processing session data for CONFER_NUM {confer_num} (Title: {main_item.get('TITLE', 'N/A')}): {e}")
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def fetch_session_details(self, session_id=None, force=False, debug=False): # Celery provides 'self'
    ...
    logger.info(f"🔍 Fetching details for session: {session_id} (force={force}, debug={debug})")
    if debug: logger.debug(f"🐛 DEBUG: Fetching details for session {session_id} in debug mode")
    ...
    try:
        ...
        params = { "KEY": settings.ASSEMBLY_API_KEY, "Type": "json", "CONF_ID": formatted_conf_id }
        ...
        if debug:
            logger.debug(f"🐛 DEBUG: VCONFDETAIL API response for {session_id}: {json.dumps(data, indent=2, ensure_ascii=False)}")

        detail_data_item = None
        api_key_name = 'VCONFDETAIL'
        if data and api_key_name in data and isinstance(data[api_key_name], list):
            if len(data[api_key_name]) > 1 and isinstance(data[api_key_name][1], dict) and 'row' in data[api_key_name][1]:
                ...
                head_info = data[api_key_name][0].get('head')
                if head_info and head_info[0].get('RESULT', {}).get('CODE', '').startswith("INFO-200"): # No data
                     logger.info(f"API result for VCONFDETAIL ({session_id}) indicates no detailed data (INFO-200).")
                elif 'row' in data[api_key_name][0]: # Fallback for inconsistent structure
                     rows = data[api_key_name][0]['row']
                     if rows and isinstance(rows, list) and len(rows) > 0:
                         detail_data_item = rows[0]

        if not detail_data_item:
            logger.info(f"ℹ️ No detailed info available from VCONFDETAIL API for session {session_id}. Might be normal.")
            ...
            if not debug: # Avoid chaining calls rapidly in debug, or make it conditional
                if is_celery_available():
                    fetch_session_bills.delay(session_id=session_id, force=force, debug=debug)
                else:
                    fetch_session_bills(session_id=session_id, force=force, debug=debug)
            return

        if debug:
            logger.debug(f"🐛 DEBUG: Would update session {session_id} with details: {detail_data_item}")
        else:
            try:
                ...
                if detail_data_item.get('CONF_TIME'):
                    try:
                        ...
                        logger.warning(f"Could not parse CONF_TIME: {detail_data_item.get('CONF_TIME')} for session {session_id}")
                ...
                if detail_data_item.get('ED_TIME'):
                    try:
                        ...
                        logger.warning(f"Could not parse ED_TIME: {detail_data_item.get('ED_TIME')} for session {session_id}")
                ...
                if detail_data_item.get('TITLE') and session_obj.title != detail_data_item.get('TITLE'):
                    session_obj.title = detail_data_item.get('TITLE')
                    updated_fields = True

                if updated_fields or force: # Save if fields changed or force is on
                    session_obj.save()
                    logger.info(f"✅ Updated session details for: {session_id} from VCONFDETAIL.")
                else:
                    logger.info(f"No changes to session details for {session_id} from VCONFDETAIL or not in force mode.")
                ...
                if session_obj.down_url: # PDF processing depends on down_url
                    if is_celery_available():
                        process_session_pdf.delay(session_id=session_id, force=force, debug=debug)
                    else:
                        process_session_pdf(session_id=session_id, force=force, debug=debug)
                else:
                    logger.info(f"No PDF URL (down_url) for session {session_id}, skipping PDF processing.")

            except Session.DoesNotExist:
                logger.error(f"❌ Session {session_id} not found in database when trying to update details. Original session creation might have failed.")
            except Exception as e_db:
                logger.error(f"❌ DB error updating session {session_id} details: {e_db}")

    except RequestException as re_exc:
        logger.error(f"Request error fetching details for session {session_id}: {re_exc}")
        try: self.retry(exc=re_exc)
        except MaxRetriesExceededError: logger.error(f"Max retries for session details {session_id}.")
    except json.JSONDecodeError as json_e:
        logger.error(f"JSON decode error for session {session_id} details: {json_e}")
        # Probably don't retry JSON errors from API, it indicates bad data
    except Exception as e:
        logger.error(f"❌ Unexpected error fetching session {session_id} details: {e}")
        logger.exception(f"Full traceback for session {session_id} detail fetch:")
        try: self.retry(exc=e)
        except MaxRetriesExceededError: logger.error(f"Max retries after unexpected error for {session_id}.")
        # raise # Optionally


@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def fetch_session_bills(self, session_id=None, force=False, debug=False): # Celery provides 'self'
    """Fetch bills for a specific session using VCONFBILLLIST API."""
    if not session_id:
        logger.error("session_id is required for fetch_session_bills.")
        return

    logger.info(f"🔍 Fetching bills for session: {session_id} (force={force}, debug={debug})")
    if debug: logger.debug(f"🐛 DEBUG: Fetching bills for session {session_id} in debug mode")
    ...
    try:
        ...
        params = { "KEY": settings.ASSEMBLY_API_KEY, "Type": "json", "CONF_ID": formatted_conf_id, "pSize": 500 }
        ...
        if debug:
             logger.debug(f"🐛 DEBUG: Full VCONFBILLLIST response for {session_id}: {json.dumps(data, indent=2, ensure_ascii=False)}")
        ...
        if data and api_key_name in data and isinstance(data[api_key_name], list):
            if len(data[api_key_name]) > 1 and isinstance(data[api_key_name][1], dict):
                ...
                if head_info and head_info[0].get('RESULT',{}).get('CODE','').startswith("INFO-200"):
                    logger.info(f"API result for VCONFBILLLIST ({session_id}) indicates no bill data (INFO-200).")
                elif 'row' in data[api_key_name][0]: # Fallback for inconsistent structure
                     bills_data_list = data[api_key_name][0].get('row', [])

        if not bills_data_list:
            logger.info(f"ℹ️ No bills found from VCONFBILLLIST API for session {session_id}.")
            return

        if debug:
            logger.debug(f"🐛 DEBUG: Found {len(bills_data_list)} bills for session {session_id} from API.")
        else:
            try:
                session_obj = Session.objects.get(conf_id=session_id)
            except Session.DoesNotExist:
                logger.error(f"❌ Session {session_id} not found in database when fetching bills. Cannot associate bills.")
                return
            ...
            for bill_item in bills_data_list:
                ...
                if not bill_id_api:
                    logger.warning(f"Skipping bill item due to missing BILL_ID in session {session_id}: {bill_item.get('BILL_NM', 'N/A')}")
                    continue
                ...
                bill_obj, created = Bill.objects.update_or_create(
                    bill_id=bill_id_api, # BILL_ID from API is the primary key for bills
                    defaults=bill_defaults
                )

                if created:
                    created_count += 1
                    logger.info(f"✨ Created new bill: {bill_id_api} ({bill_obj.bill_nm[:30]}...) for session {session_id}")
                else: # Bill already existed, update_or_create updated it
                    updated_count +=1
                    logger.info(f"🔄 Updated existing bill: {bill_id_api} ({bill_obj.bill_nm[:30]}...) for session {session_id}")
            logger.info(f"🎉 Bills processed for session {session_id}: {created_count} created, {updated_count} updated.")

    except RequestException as re_exc:
        logger.error(f"Request error fetching bills for session {session_id}: {re_exc}")
        try: self.retry(exc=re_exc)
        except MaxRetriesExceededError: logger.error(f"Max retries for session bills {session_id}.")
    except json.JSONDecodeError as json_e:
        logger.error(f"JSON decode error for session {session_id} bills: {json_e}")
    except Exception as e:
        logger.error(f"❌ Unexpected error fetching bills for session {session_id}: {e}")
        logger.exception(f"Full traceback for session {session_id} bill fetch:")
        try: self.retry(exc=e)
        except MaxRetriesExceededError: logger.error(f"Max retries after unexpected error for bills of {session_id}.")
        # raise # Optionally
...
def get_session_bills_list(session_id):
    ...
    try:
        ...
        params = {
            'KEY': settings.ASSEMBLY_API_KEY, # Or settings.OPEN_ASSEMBLY_API_KEY
            'Type': 'json',
            'pIndex': 1,
            'pSize': 1000, # Max allowed, or paginate if more
            'CONF_NUM': str(session_id) # API expects string, ensure session_id is appropriate for CONF_NUM
        }
        ...
        if data and api_key_name_bills in data and isinstance(data[api_key_name_bills], list):
            if len(data[api_key_name_bills]) > 1 and isinstance(data[api_key_name_bills][1], dict):
        ...
        logger.info(f"Found {len(bill_names)} bill names for CONF_NUM {session_id} via nwvrqwxyaytdsfvhu.")
        return bill_names

    except requests.exceptions.RequestException as e:
        logger.error(f"Network error getting bills for session/conf_num {session_id} (nwvrqwxyaytdsfvhu): {e}")
    except json.JSONDecodeError as e:
        logger.error(f"JSON parsing error for bills of session/conf_num {session_id} (nwvrqwxyaytdsfvhu): {e}")
    except Exception as e:
        logger.error(f"Unexpected error getting bill names for session/conf_num {session_id} (nwvrqwxyaytdsfvhu): {e}")
    return []
...
def get_session_bill_names(session_id):
    ...
    try:
        ...
        bills = Bill.objects.filter(session=session).values_list('bill_nm', flat=True)
        return list(bill for bill in bills if bill) # Filter out empty/None names
    except Session.DoesNotExist:
        logger.error(f"Session {session_id} not found when trying to get its bill names from DB.")
    except Exception as e:
        logger.error(f"❌ Error fetching stored bill names for session {session_id}: {e}")
    return []
...
def extract_text_segment(text, start_marker, end_marker=None):
    ...
    try:
        ...
        if start_marker:
            ...
            start_pos = found_pos + len(start_marker) # Segment starts AFTER the marker
        ...
        logger.error(f"❌ Error extracting text segment ('{start_marker}' to '{end_marker}'): {e}")
        return ""


def extract_statements_for_bill_segment(bill_text_segment, session_id, bill_name, debug=False):
    """Extract and analyze statements for a specific bill text segment using LLM."""
    if not bill_text_segment: return []

    logger.info(f"🔍 Stage 1 (Speaker Detect): For bill '{bill_name}' (session: {session_id})")

    if not genai or not hasattr(genai, 'GenerativeModel'):
        logger.warning("❌ Gemini API not configured. Cannot perform speaker detection for bill segment.")
        return []

    try:
        # Use a lighter/cheaper model for speaker detection stage if appropriate
        speaker_detection_model_name = 'gemini-1.5-flash-latest' # Or 'gemini-2.0-flash-lite' if still available & preferred
        speaker_detection_llm = genai.GenerativeModel(speaker_detection_model_name)
    except Exception as e_model:
        logger.error(f"Failed to initialize speaker detection model ({speaker_detection_model_name}): {e_model}")
        return []

    # Limit text length for prompts
...
"""
    ...
    try:
        stage1_response = speaker_detection_llm.generate_content(speaker_detection_prompt)
        if not stage1_response or not stage1_response.text:
            logger.warning(f"❌ No response from LLM speaker detection for bill '{bill_name}'.")
            return []

        response_text_cleaned = stage1_response.text.strip().replace("```json", "").replace("```", "").strip()

        try:
            stage1_data = json.loads(response_text_cleaned)
        except json.JSONDecodeError as e:
            logger.error(f"❌ JSON parsing error for speaker detection (bill '{bill_name}'): {e}. Response was: {response_text_cleaned}")
            return []

        detected_speeches_info = stage1_data.get('detected_speeches', [])
        logger.info(f"✅ Speaker detection for '{bill_name}': Found {len(detected_speeches_info)} potential speech segments.")
        ...
        for i, speech_info in enumerate(detected_speeches_info):
            ...
            if not clean_name or not start_cue or not is_real_person or not is_substantial:
                logger.info(f"Skipping speech segment for '{bill_name}' due to missing info or filters: Name='{clean_name}', Cue='{start_cue}', Person={is_real_person}, Substantial={is_substantial}")
                continue
            ...
            if start_idx == -1:
                logger.warning(f"Could not find start_cue '{start_cue}' for speaker '{clean_name}' in bill_text_segment for '{bill_name}'. Skipping.")
                continue
            ...
            if i + 1 < len(detected_speeches_info):
                ...
                if next_start_cue:
                    found_next_cue_at = bill_text_segment.find(next_start_cue, start_idx + 1) # Search after current cue
            ...
            if current_speech_content.startswith(speech_info.get('speaker_name_raw','')):
                 current_speech_content = current_speech_content[len(speech_info.get('speaker_name_raw','')):].strip()
            elif current_speech_content.startswith(start_cue): # if start_cue includes the name
                 # this logic is tricky, relies on good cues from LLM
                 pass # The cue itself IS the start of the text LLM saw.

            if not current_speech_content or len(current_speech_content) < 50: # Min length for meaningful analysis
                logger.info(f"Skipping short/empty speech by '{clean_name}' for '{bill_name}'.")
                continue

            logger.info(f"Analyzing content for '{clean_name}' on bill '{bill_name}' (approx {len(current_speech_content)} chars).")
            # Analyze this specific speech content with bill context
            analysis_result_dict = analyze_single_statement_with_bill_context(
                {'speaker_name': clean_name, 'text': current_speech_content},
        ...
        logger.error(f"❌ Error during speaker detection/analysis for bill '{bill_name}': {e}")
        logger.exception("Full traceback for bill segment processing error:")

    logger.info(f"✅ Bill segment '{bill_name}' analysis resulted in {len(analyzed_statements_for_bill)} statements.")
    return analyzed_statements_for_bill


def analyze_single_statement_with_bill_context(statement_data_dict, session_id, bill_name, debug=False):
    ...
    if not model: # Global 'model' for detailed analysis (e.g., gemma-3)
        logger.warning("❌ Main LLM ('model') not available. Cannot analyze statement for bill context.")
        # Return basic structure with indication of failure
        statement_data_dict.update({
            'sentiment_score': 0.0, 'sentiment_reason': 'LLM N/A',
            'bill_relevance_score': 0.0, 'policy_categories': [],
            'policy_keywords': [], 'bill_specific_keywords': []
        })
    ...
    if not text_to_analyze:
        logger.warning(f"No text to analyze for speaker '{speaker_name}' regarding bill '{bill_name}'.")
        return statement_data_dict

    # Limit text for LLM prompt
    prompt_text_limit = 7800 # For main model
    if len(text_to_analyze) > prompt_text_limit:
        logger.warning(f"Text for '{speaker_name}' on '{bill_name}' truncated from {len(text_to_analyze)} to {prompt_text_limit} for analysis.")
...
"""
    try:
        response = model.generate_content(prompt) # Uses the global 'model' (e.g. gemma-3)
        if not response or not response.text:
            logger.warning(f"❌ No LLM analysis response for '{speaker_name}' on bill '{bill_name}'.")
        ...
        statement_data_dict.update({
            ...
            'policy_keywords': analysis_json.get('key_policy_phrases', []), # Matched to prompt output key
            'bill_specific_keywords': analysis_json.get('bill_specific_keywords_found', []) # Matched to prompt output key
        })
        if debug:
            logger.debug(f"🐛 DEBUG: Analyzed '{speaker_name}' on '{bill_name}' - Sent: {statement_data_dict['sentiment_score']}, BillRel: {statement_data_dict['bill_relevance_score']}")
        return statement_data_dict
    except json.JSONDecodeError as e:
        logger.error(f"❌ JSON parsing error for LLM analysis ('{speaker_name}' on '{bill_name}'): {e}. Response: {response_text_cleaned if 'response_text_cleaned' in locals() else 'N/A'}")
    except Exception as e:
        logger.error(f"❌ Error during LLM analysis of statement for '{speaker_name}' on bill '{bill_name}': {e}")
...
def extract_statements_without_bill_separation(full_text, session_id, bills_context_str, debug=False):
    """Fallback: LLM speaker detection on full text, then individual analysis if no bill segmentation."""
    logger.info(f"🔄 Using full-text speaker detection for session: {session_id} (no bill segmentation).")

    if not genai or not hasattr(genai, 'GenerativeModel'):
        logger.warning("❌ Gemini API not configured. Cannot perform full-text speaker detection.")
        return []
    try:
        speaker_detection_model_name = 'gemini-1.5-flash-latest' # Or 'gemini-2.0-flash-lite'
        speaker_detection_llm = genai.GenerativeModel(speaker_detection_model_name)
    except Exception as e_model:
        logger.error(f"Failed to initialize speaker detection model ({speaker_detection_model_name}): {e_model}")
        return []
    ...
    if len(full_text) > prompt_text_limit:
        logger.warning(f"Full text for session {session_id} truncated for speaker detection prompt.")
...
"""
    ...
    try:
        stage1_response = speaker_detection_llm.generate_content(speaker_detection_prompt)
        if not stage1_response or not stage1_response.text:
            logger.warning(f"No response from full-text speaker detection LLM for session {session_id}.")
            return []

        response_text_cleaned = stage1_response.text.strip().replace("```json", "").replace("```", "").strip()
        stage1_data = json.loads(response_text_cleaned)
        detected_speeches_info = stage1_data.get('detected_speeches', [])
        logger.info(f"Full-text speaker detection for session {session_id}: Found {len(detected_speeches_info)} potential speech segments.")
        ...
        for i, speech_info in enumerate(detected_speeches_info):
            ...
            if i + 1 < len(detected_speeches_info):
                next_start_cue = detected_speeches_info[i+1].get('speech_start_cue')
                if next_start_cue:
                    found_next_cue_at = full_text.find(next_start_cue, start_idx + 1)
            ...
            if not current_speech_content or len(current_speech_content) < 50: continue
            ...
            if analysis_result_dict: # analyze_single_statement should return a dict
                ...
                analysis_result_dict['associated_bill_name'] = "General Discussion / Full Transcript"
                all_analyzed_statements.append(analysis_result_dict)

            if not debug: time.sleep(0.6)

    except json.JSONDecodeError as e:
        logger.error(f"JSON parsing error in full-text speaker detection (session {session_id}): {e}. Response: {response_text_cleaned if 'response_text_cleaned' in locals() else 'N/A'}")
    except Exception as e:
        logger.error(f"❌ Error in full-text statement extraction (session {session_id}): {e}")
        logger.exception("Full traceback for full-text extraction error:")

    logger.info(f"✅ Full-text LLM extraction for session {session_id} completed: {len(all_analyzed_statements)} statements.")
    return all_analyzed_statements


@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def process_session_pdf(self, session_id=None, force=False, debug=False): # Celery provides 'self'
    ...
    if not session.down_url:
        logger.info(f"ℹ️ No PDF URL (down_url) for session {session_id}. Skipping PDF processing.")
        return

    if Statement.objects.filter(session=session).exists() and not force and not debug:
        logger.info(f"Statements already exist for session {session_id} and not in force/debug mode. Skipping PDF reprocessing.")
        return

    if debug:
        logger.debug(f"🐛 DEBUG: Simulating PDF processing for {session_id}. NOT downloading or parsing in debug.")
    ...
    try:
        ...
        response = requests.get(session.down_url, timeout=120, stream=True) # Increased timeout for large PDFs
        response.raise_for_status()

        temp_dir = Path(getattr(settings, "TEMP_FILE_DIR", "temp_files")) # Use configurable temp dir
        ...
        try:
            with pdfplumber.open(temp_pdf_path) as pdf:
                ...
                if pdf.is_encrypted:
                    logger.warning(f"PDF {temp_pdf_path} is encrypted. Trying to extract text...")
                    # pdfplumber might need password if it's owner-password encrypted against text extraction

                pages = pdf.pages
                logger.info(f"Extracting text from {len(pages)} pages in PDF for session {session_id}...")
                for i, page in enumerate(pages):
                    page_text = page.extract_text(x_tolerance=1, y_tolerance=3) # Adjust tolerances
                    if page_text:
                        full_text += page_text + "\n"
                    if (i+1) % 20 == 0: logger.info(f"Processed {i+1}/{len(pages)} pages...")
            logger.info(f"📄 Extracted ~{len(full_text)} chars from PDF for session {session_id}.")
        except Exception as e_parse: # Catch pdfplumber specific errors
            logger.error(f"❌ Error extracting text using pdfplumber for {session_id} from {temp_pdf_path}: {e_parse}")
            if "Incorrect থানা opening times password" in str(e_parse): # Specific error for some password types
                 logger.error("PDF might be password protected against text extraction.")
            return # Stop if text extraction fails

        if not full_text.strip():
            logger.warning(f"Extracted text is empty for session {session_id}. PDF might be image-based or have extraction issues.")
            return

        # Fetch bill context (names of bills discussed in this session) for LLM
        bills_for_session = get_session_bill_names(session_id) # Uses DB Bill objects
        bills_context_str = ", ".join(bills_for_session) if bills_for_session else "여러 의안"
        logger.info(f"Bills context for session {session_id} LLM: {bills_context_str[:200]}...")
        ...
        process_pdf_text_for_statements(full_text, session_id, session, bills_context_str, bills_for_session, debug)

    except RequestException as re_exc:
        logger.error(f"Request error downloading PDF for session {session_id}: {re_exc}")
        try: self.retry(exc=re_exc)
        except MaxRetriesExceededError: logger.error(f"Max retries for PDF download {session_id}.")
    except Exception as e:
        ...
        try: self.retry(exc=e) # Retry for other unexpected errors too
        except MaxRetriesExceededError: logger.error(f"Max retries after unexpected PDF error {session_id}.")
        # raise # Optionally
    finally:
...
@shared_task(bind=True, max_retries=1, default_retry_delay=300) # Less retries for costly LLM task
def analyze_statement_categories(self, statement_id=None): # Celery provides 'self'
    """Analyze categories and sentiment for an existing statement using LLM. (Usually part of initial processing)"""
    if not statement_id:
        logger.error("statement_id is required for analyze_statement_categories.")
        return

    if not model: # Global 'model'
        logger.warning("❌ Main LLM ('model') not available. Cannot analyze statement categories.")
        return

    try:
        statement = Statement.objects.get(id=statement_id)
        # Potentially skip if already analyzed to a satisfactory degree, unless forced
        if statement.sentiment_score is not None and statement.category_analysis and not getattr(self, 'force_reanalyze', False):
            logger.info(f"Statement {statement_id} already analyzed. Skipping re-analysis.")
            return

    except Statement.DoesNotExist:
        logger.error(f"Statement with id {statement_id} not found for analysis.")
        return

    logger.info(f"Analyzing categories for statement ID: {statement_id} by {statement.speaker.naas_nm}")
...
"""
    ...
    try:
        ...
        if not response or not response.text:
            logger.warning(f"❌ No LLM response for category analysis of statement {statement_id}")
            return

        response_text_cleaned = response.text.strip().replace("```json", "").replace("```", "").strip()
        analysis_json = json.loads(response_text_cleaned)

        statement.sentiment_score = analysis_json.get('sentiment_score', statement.sentiment_score) # Keep old if new is missing
        statement.sentiment_reason = analysis_json.get('sentiment_reason', statement.sentiment_reason)
        statement.policy_keywords = ', '.join(analysis_json.get('key_policy_phrases', []))

        policy_categories_json = analysis_json.get('policy_categories', [])
        statement.category_analysis = json.dumps(policy_categories_json, ensure_ascii=False) if policy_categories_json else statement.category_analysis
        ...
        logger.info(f"✅ LLM Category analysis completed for statement {statement_id}.")

    except json.JSONDecodeError as e:
        logger.error(f"❌ JSON parsing error for LLM category analysis (statement {statement_id}): {e}. Response: {response_text_cleaned if 'response_text_cleaned' in locals() else 'N/A'}")
    except Exception as e:
        logger.error(f"❌ Error analyzing categories for statement {statement_id}: {e}")
        logger.exception("Full traceback for category analysis error:")
        try: self.retry(exc=e)
        except MaxRetriesExceededError: logger.error(f"Max retries for category analysis {statement_id}.")
        # raise # Optionally
...
def process_pdf_text_for_statements(full_text, session_id, session_obj, bills_context_str, bill_names_list, debug=False):
    ...
    logger.info(f"🤖 Starting LLM-based statement processing for session PDF {session_id}.")

    all_extracted_statements_data = [] # List of dicts, each a statement with analysis

    # Stage 0: Bill Segmentation (optional, but preferred if bills_names_list is rich)
    # Using a lighter model for segmentation.
    try:
        segmentation_model_name = 'gemini-1.5-flash-latest'
        segmentation_llm = genai.GenerativeModel(segmentation_model_name)
    except Exception as e_model:
        logger.error(f"Failed to initialize segmentation model ({segmentation_model_name}): {e_model}. Will process full text.")
        segmentation_llm = None


    bill_segments_from_llm = []
    if segmentation_llm and bill_names_list and len(bill_names_list) > 1: # Only segment if multiple bills context
        logger.info(f"🔍 Stage 0 (Bill Segment): Attempting to segment transcript by bills for session {session_id}")
...
"""
        try:
            ...
            if seg_response and seg_response.text:
                seg_text_cleaned = seg_response.text.strip().replace("```json","").replace("```","").strip()
                seg_data = json.loads(seg_text_cleaned)
                bill_segments_from_llm = seg_data.get("bill_discussion_segments", [])
            ...
            logger.error(f"JSON parsing error for bill segmentation response: {e_json_seg}. Processing full text.")
        except Exception as e_seg:
            logger.error(f"Error during LLM bill segmentation: {e_seg}. Processing full text.")
    ...
    if sorted_segments_with_text:
        logger.info(f"Processing {len(sorted_segments_with_text)} identified bill text segments...")
        for seg_data in sorted_segments_with_text:
            bill_name_for_seg = seg_data["bill_name"]
            text_of_segment = seg_data["text"]
            logger.info(f"--- Processing segment for Bill: {bill_name_for_seg} ({len(text_of_segment)} chars) ---")
            ...
            for stmt_data in statements_in_segment:
                stmt_data['associated_bill_name'] = bill_name_for_seg # Add association
            all_extracted_statements_data.extend(statements_in_segment)
            if not debug: time.sleep(1) # Pause between processing major segments
    else:
        logger.info("No bill segments identified or processed. Processing PDF text as a single unit.")
    ...
    logger.info(f"Collected a total of {len(all_extracted_statements_data)} analyzed statements for session {session_id}.")
    if not debug and all_extracted_statements_data:
        ...
        logger.debug(f"🐛 DEBUG: Would save {len(all_extracted_statements_data)} statements. First one: {all_extracted_statements_data[0] if all_extracted_statements_data else 'None'}")


def process_extracted_statements_data(statements_data_list, session_obj, debug=False):
    """Saves a list of processed statement data (dictionaries) to the database."""
    if debug:
        logger.debug(f"🐛 DEBUG: Would process {len(statements_data_list)} statement data items. Not saving to DB.")
        return

    if not statements_data_list:
        logger.info(f"No statement data to save for session {session_obj.conf_id}.")
        return

    created_count = 0
    logger.info(f"Attempting to save {len(statements_data_list)} statements for session {session_obj.conf_id}.")
    for stmt_data in statements_data_list:
        try:
            ...
            if not speaker_name or not statement_text:
                logger.warning(f"Skipping statement with missing speaker ('{speaker_name}') or text for session {session_obj.conf_id}.")
                continue
            ...
            speaker_obj = get_or_create_speaker(speaker_name, debug=debug) # Debug here should match overall debug
            if not speaker_obj:
                logger.warning(f"⚠️ Could not get/create speaker: {speaker_name}. Skipping statement.")
                continue

            # Check for existing identical statement (text, speaker, session) to avoid duplicates from reprocessing
            if Statement.objects.filter(session=session_obj, speaker=speaker_obj, text_hash=Statement.calculate_hash(statement_text)).exists():
                logger.info(f"ℹ️ Identical statement by {speaker_name} (hash match) already exists for session {session_obj.conf_id}. Skipping.")
                continue

            associated_bill_obj = None
            assoc_bill_name_from_data = stmt_data.get('associated_bill_name') # Set during segmentation/full_text processing
            if assoc_bill_name_from_data and assoc_bill_name_from_data not in ["General Discussion / Full Transcript", "Unknown Bill Segment"]:
                # Try to find the Bill object precisely
                try:
                    # Prefer exact match if possible, or high-confidence partial
                    associated_bill_obj = Bill.objects.filter(session=session_obj, bill_nm__iexact=assoc_bill_name_from_data).first()
                    if not associated_bill_obj:
                        ...
                        bill_candidates = Bill.objects.filter(session=session_obj, bill_nm__icontains=assoc_bill_name_from_data.split('(')[0].strip()) # Match before parenthesis
                        if bill_candidates.count() == 1:
                            associated_bill_obj = bill_candidates.first()
                        elif bill_candidates.count() > 1:
                            logger.warning(f"Ambiguous bill name '{assoc_bill_name_from_data}' for session {session_obj.conf_id}, found multiple matches. Not associating.")


                except Exception as e_bill_find:
                    logger.warning(f"⚠️ Error finding bill '{assoc_bill_name_from_data}' for statement: {e_bill_find}")
            ...
            new_statement = Statement(
                ...
                sentiment_reason=stmt_data.get('sentiment_reason', 'Analysis not fully run'),
                category_analysis=json.dumps(stmt_data.get('policy_categories', []), ensure_ascii=False),
                policy_keywords=', '.join(stmt_data.get('policy_keywords', [])),
                # Add new fields if your model has them
                bill_relevance_score = stmt_data.get('bill_relevance_score'), # May be None
                bill_specific_keywords_json = json.dumps(stmt_data.get('bill_specific_keywords',[]), ensure_ascii=False)
            )
            new_statement.save() # This also calculates and saves text_hash via pre_save signal
            created_count += 1
            ...
            logger.info(f"✨ Created statement ({new_statement.id}) for {speaker_name}{bill_info_log}: {statement_text[:40]}...")

            if stmt_data.get('policy_categories'):
                create_statement_categories(new_statement, stmt_data['policy_categories'])

        except Exception as e_stmt_save:
            logger.error(f"❌ Error creating statement object in DB: {e_stmt_save} for speaker {stmt_data.get('speaker_name', 'N/A')}")
            logger.error(f"Failing statement data: {json.dumps(stmt_data, ensure_ascii=False, indent=2)}")
            logger.exception("Traceback for statement save error:")
            continue # Continue with the next statement

    logger.info(f"🎉 Saved {created_count} new statements for session {session_obj.conf_id}.")


def extract_statements_with_llm_validation(full_text, session_id, bills_context_str, debug=False):
    """
    ...
    """
    logger.warning("`extract_statements_with_llm_validation` called directly. Processing full text without segmentation attempt.")
    if not model:
        logger.warning("❌ LLM model not available, using very basic regex fallback if any.")
        # return extract_statements_with_regex_fallback(full_text, session_id, debug) # This fallback also needs LLM or heavy rework
        return []

    return extract_statements_without_bill_separation(full_text, session_id, bills_context_str, debug)


def extract_speech_between_markers(full_text, exact_start_marker, next_speaker_char_cue="◯", speaker_name_for_log="Unknown"):
    """
    ...
    try:
        ...
        if start_pos == -1:
            logger.warning(f"Could not find exact_start_marker '{exact_start_marker}' for {speaker_name_for_log}.")
            return ""
        ...
        search_for_next_cue_from = start_pos + len(exact_start_marker) # Search AFTER the current marker text
        if search_for_next_cue_from >= len(full_text): # If current marker is at the very end
             search_for_next_cue_from = start_pos + 1 # At least search 1 char after current marker's start


        next_cue_pos = full_text.find(next_speaker_char_cue, search_for_next_cue_from)
        ...
        import re
        speech_content_cleaned = re.sub(r'\s*\([^)]*\)\s*', ' ', speech_content) # remove (text) and surrounding spaces
        speech_content_cleaned = re.sub(r'\s+', ' ', speech_content_cleaned).strip() # Consolidate multiple spaces

        return speech_content_cleaned

    except Exception as e:
        logger.error(f"❌ Error in extract_speech_between_markers for '{speaker_name_for_log}': {e}")
        return ""
...
def extract_statements_with_regex_fallback(text, session_id, debug=False):
    ...
    """
    import re
    logger.warning(f"⚠️ Using basic regex fallback for statement extraction (session: {session_id}). Results will be very rough.")

    cleaned_text = re.sub(r'\n+', '\n', text).replace('\r', '') # Normalize newlines
        ...
        if line_stripped.startswith("◯"):
            if current_speaker and current_speech_lines: # Save previous speaker's utterance
                statements.append({
                    ...
                    'sentiment_score': 0.0, 'sentiment_reason': 'Regex Fallback',
                    'policy_categories': [], 'policy_keywords': []
                })
                current_speech_lines = []

            # Try to parse new speaker
            speaker_part = line_stripped[1:].split(' ')[0] # Very naive speaker extraction
    ...
    if current_speaker and current_speech_lines:
        statements.append({
            'speaker_name': current_speaker, 'text': " ".join(current_speech_lines).strip(),
            'sentiment_score': 0.0, 'sentiment_reason': 'Regex Fallback',
            'policy_categories': [], 'policy_keywords': []
        })

    logger.info(f"Regex fallback completed: Extracted {len(statements)} potential statement blocks for session {session_id}.")
...
"""
    try:
        ...
        logger.error(f"❌ JSON parsing error for LLM generic analysis ('{speaker_name}'): {e}. Response: {response_text_cleaned if 'response_text_cleaned' in locals() else 'N/A'}")
    except Exception as e:
        logger.error(f"❌ Error during LLM generic analysis of statement for '{speaker_name}': {e}")
    return statement_data_dict
...
def get_bills_context(session_id):
    ...
    try:
        ...
        logger.error(f"❌ Error fetching bills context string for session {session_id}: {e}")
        return "의안 목록 조회 중 오류 발생"


def parse_and_analyze_statements_from_text(full_text, session_id, bills_context_str, debug=False):
    """
    ...
    """
    logger.warning("`parse_and_analyze_statements_from_text` called. "
    ...
    logger.info(f"✅ Full-text parsing and analysis yielded {len(statements_data)} statements for session {session_id}.")
    return statements_data # Returns list of dicts with analysis


def create_statement_categories(statement_obj, policy_categories_list_from_llm):
    ...
    for cat_data in policy_categories_list_from_llm:
        ...
        confidence = float(cat_data.get('confidence', 0.5)) # Default confidence

        if not main_cat_name:
            continue

        # Avoid duplicate (Main, Sub) for the same statement
        category_tuple = (main_cat_name, sub_cat_name if sub_cat_name else "일반")
        ...
        try:
            ...
            if sub_cat_name and sub_cat_name.lower() != '일반' and sub_cat_name.lower() != '없음':
                subcategory_obj, _ = Subcategory.objects.get_or_create(
                    name=sub_cat_name,
                    category=category_obj, # Associate with parent category
                    defaults={'description': f'{sub_cat_name} 관련 세부 정책 ({main_cat_name})'}
                )
            ...
            StatementCategory.objects.update_or_create(
                ...
                subcategory=subcategory_obj, # Can be None if no specific subcategory
                defaults={'confidence_score': confidence}
            )
        except Exception as e_cat_create:
            logger.error(f"Error creating category links for statement {statement_obj.id} (Cat: {main_cat_name}/{sub_cat_name}): {e_cat_create}")
            continue
    logger.info(f"Updated category associations for statement {statement_obj.id}.")
...
def get_or_create_speaker(speaker_name_raw, debug=False):
    ...
    if not speaker_name_raw or not speaker_name_raw.strip():
        logger.warning("Empty speaker_name_raw provided to get_or_create_speaker.")
        return None
    ...
    if not speaker_name_cleaned: # If stripping resulted in empty name
        logger.warning(f"Speaker name '{speaker_name_raw}' became empty after cleaning.")
        return None
    ...
    try:
        ...
        speaker_obj = Speaker.objects.filter(naas_nm=speaker_name_cleaned).first()

        if speaker_obj:
            if debug: logger.debug(f"Found existing speaker: {speaker_name_cleaned}")
            return speaker_obj
        ...
        logger.info(f"Speaker '{speaker_name_cleaned}' not found in DB. Attempting to fetch details from API.")
        ...
        if not debug: # Avoid external API calls in some debug scenarios for speed/cost
            ...
            if speaker_obj_from_api:
                logger.info(f"Successfully fetched/created speaker from API: {speaker_obj_from_api.naas_nm}")
                return speaker_obj_from_api
            else:
                logger.warning(f"Failed to fetch details for new speaker '{speaker_name_cleaned}' from API.")
        ...
        speaker_obj, created = Speaker.objects.get_or_create(
            ...
            defaults={ # Provide all required non-nullable fields with placeholders
                ...
                'naas_ch_nm': '', 'plpt_nm': '정보없음', 'elecd_nm': '', 'elecd_div_nm': '',
                'cmit_nm': '', 'blng_cmit_nm': '', 'rlct_div_nm': '', 'gtelt_eraco': '',
                'ntr_div': '', 'naas_pic': ''
            }
        )
        if created:
            logger.info(f"Created basic/temporary speaker record for: {speaker_name_cleaned} (ID: {speaker_obj.naas_cd}). Details might be incomplete.")
        else: # Should not happen if previous checks were exhaustive, but good fallback
            logger.info(f"Found speaker {speaker_name_cleaned} via get_or_create after API attempt.")
        return speaker_obj

    except Exception as e:
        logger.error(f"❌ Error in get_or_create_speaker for '{speaker_name_raw}': {e}")
...
def fetch_additional_data_nepjpxkkabqiqpbvk(self, force=False, debug=False): # Celery provides 'self'
    ...
    logger.info(f"🔍 Fetching additional data from {api_endpoint_name} API (force={force}, debug={debug})")

    if debug:
        logger.debug(f"🐛 DEBUG: Skipping actual API call for {api_endpoint_name} in debug mode.")
        return

    try:
        if not hasattr(settings, 'ASSEMBLY_API_KEY') or not settings.ASSEMBLY_API_KEY:
            logger.error(f"ASSEMBLY_API_KEY not configured for {api_endpoint_name}.")
            return
        ...
        while current_page <= max_pages:
            params = {
                "KEY": settings.ASSEMBLY_API_KEY, "Type": "json",
                "pIndex": current_page, "pSize": page_size
                # Add other API-specific parameters if needed (e.g., date range, DAE_NUM)
            }
            logger.info(f"Fetching page {current_page} from {api_endpoint_name} with params: {params}")
            ...
            items_on_page = []
            if data and api_endpoint_name in data and isinstance(data[api_endpoint_name], list):
                if len(data[api_endpoint_name]) > 1 and isinstance(data[api_endpoint_name][1], dict):
                    items_on_page = data[api_endpoint_name][1].get('row', [])
                elif len(data[api_endpoint_name]) > 0 and isinstance(data[api_endpoint_name][0], dict):
                    head_info = data[api_endpoint_name][0].get('head')
                    if head_info and head_info[0].get('RESULT',{}).get('CODE','').startswith("INFO-200"): # No more data
                        logger.info(f"API result for {api_endpoint_name} (page {current_page}) indicates no more data.")
                        break # End pagination
                    elif 'row' in data[api_endpoint_name][0]:
                        items_on_page = data[api_endpoint_name][0].get('row', [])

            if not items_on_page:
                logger.info(f"No items found on page {current_page} for {api_endpoint_name}. Ending pagination.")
                break # End pagination if no items or API indicates end of data

            all_items.extend(items_on_page)
            logger.info(f"Fetched {len(items_on_page)} items from page {current_page}. Total so far: {len(all_items)}.")

            # Check if this was the last page (e.g., if less items than pSize returned)
            if len(items_on_page) < page_size:
                logger.info("Fetched less items than page size, assuming last page.")
                break
        ...
        if not all_items:
            logger.info(f"ℹ️  No data items found from {api_endpoint_name} API after checking pages.")
            return

        logger.info(f"✅ Found a total of {len(all_items)} items from {api_endpoint_name} API.")
        ...
        for item in all_items:
            try:
                ...
                logger.debug(f"Processing item (placeholder): {str(item)[:200]}...")
                processed_count += 1
            except Exception as e_item:
                logger.error(f"❌ Error processing item from {api_endpoint_name}: {e_item}. Item: {str(item)[:100]}")
                continue

        logger.info(f"🎉 Processed {processed_count} items from {api_endpoint_name} API.")

    except RequestException as re_exc:
        logger.error(f"Request error fetching from {api_endpoint_name} API: {re_exc}")
        try: self.retry(exc=re_exc)
        except MaxRetriesExceededError: logger.error(f"Max retries for {api_endpoint_name} fetch.")
    except json.JSONDecodeError as json_e:
        logger.error(f"JSON decode error from {api_endpoint_name} API: {json_e}")
    except Exception as e:
        logger.error(f"❌ Unexpected error fetching/processing from {api_endpoint_name} API: {e}")
        logger.exception(f"Full traceback for {api_endpoint_name} error:")
        try: self.retry(exc=e)
        except MaxRetriesExceededError: logger.error(f"Max retries after unexpected error for {api_endpoint_name}.")
        # raise # Optionally