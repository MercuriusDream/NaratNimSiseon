...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('🔄 Synchronizing party and member data from Assembly API...'))

        # 1. Fetch and update Speaker/member data
        self.fetch_and_parse_members(force=force, debug=debug)

        # 2. Create/update Party records from Speaker data
        self.stdout.write('🔄 Synchronizing Party records from Speaker data...')
        self.create_parties_from_existing_speakers()

        # 3. Update Speaker-Party relationships
        self.stdout.write('🔄 Updating Speaker-Party relationships...')
        self.update_speaker_party_relationships()

        # 4. Optionally show statistics
        if show_stats:
            self.show_party_member_stats()

        self.stdout.write(self.style.SUCCESS('✅ Party and member sync complete!'))
        ...
        for i in range(0, total_members, batch_size):
            ...
            for member_data in batch:
                try:
                    ...
                    # --- Robust Speaker field parsing and creation ---
                    # Parse party history (always as list)
                    plpt_nm_raw = member_data.get('PLPT_NM', '정당정보없음').strip()
                    party_list = [p.strip() for p in plpt_nm_raw.split('/') if p.strip()] if plpt_nm_raw and plpt_nm_raw != '정당정보없음' else []
                    current_party_name = party_list[-1] if party_list else '정당정보없음'

                    # Ensure current_party is a valid FK (Party)
                    from api.models import Party
                    current_party_obj = None
                    if current_party_name != '정당정보없음':
                        try:
                            current_party_obj, _ = Party.objects.get_or_create(
                                name=current_party_name,
                                defaults={'description': f'{current_party_name} - 자동 생성', 'slogan': '', 'logo_url': ''}
                            )
                        except Exception as e:
                            self.stdout.write(self.style.ERROR(f'❌ Error creating/fetching Party for {current_party_name}: {e}'))
                            current_party_obj = None

                    # Parse all list fields robustly
                    def split_list(val, sep):
                        if not val or not isinstance(val, str):
                            return []
                        return [v.strip() for v in val.split(sep) if v.strip()]

                    # Defensive: Always output lists for JSONField fields
                    elecd_nm = split_list(member_data.get('ELECD_NM'), '/')
                    elecd_div_nm = split_list(member_data.get('ELECD_DIV_NM'), '/')
                    cmit_nm = split_list(member_data.get('CMIT_NM'), ',')
                    blng_cmit_nm = split_list(member_data.get('BLNG_CMIT_NM'), ',')

                    # Defensive: Always output list of ints for gtelt_eraco
                    # GTELT_ERACO: parse as list of ints
                    import re
                    gtelt_eraco_raw = member_data.get('GTELT_ERACO', '')
                    gtelt_eraco = [int(num) for num in re.findall(r'제(\d+)대', gtelt_eraco_raw)] if gtelt_eraco_raw else []
                    # RLCT_DIV_NM: extract integer from string like '3선'
                    rlct_div_nm_raw = member_data.get('RLCT_DIV_NM', '')
                    nth_term = int(re.sub(r'[^0-9]', '', rlct_div_nm_raw)) if rlct_div_nm_raw and re.search(r'\d+', rlct_div_nm_raw) else None

                    # Defensive: ensure naas_ch_nm is never null/empty
                    naas_ch_nm = member_data.get('NAAS_CH_NM')
                    if not naas_ch_nm:
                        naas_ch_nm = '정보없음'

                    # Defensive: ensure ntr_div is never null/empty
                    ntr_div = member_data.get('NTR_DIV')
                    if not ntr_div:
                        ntr_div = '정보없음'

                    # Defensive: ensure naas_pic is always a string
                    ...
                    try:
                        speaker, created = Speaker.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                'plpt_nm': plpt_nm_raw,  # Full party string for party history
                                'current_party': current_party_obj,
                                'elecd_nm': elecd_nm,
                                'elecd_div_nm': elecd_div_nm,
                                'cmit_nm': cmit_nm,
                                'blng_cmit_nm': blng_cmit_nm,
                                'gtelt_eraco': gtelt_eraco,
                                'era_int': gtelt_eraco[-1] if gtelt_eraco else None,
                                'nth_term': nth_term,
                        ...
                        self.stdout.write(self.style.ERROR(f'❌ Error creating/updating Speaker {member_data.get('NAAS_NM', 'Unknown')} ({naas_cd}): {e}'))
                        continue

                    # Optionally, store party history for later use (not DB)
                    speaker.party_history_cache = party_list
