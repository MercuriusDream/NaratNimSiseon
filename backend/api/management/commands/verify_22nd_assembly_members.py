...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        official_parties = {
            '국민의힘', '더불어민주당', '조국혁신당', '개혁신당', '진보당', '기본소득당', '사회민주당', '무소속'
        }

        self.stdout.write(
            self.style.SUCCESS('🔍 Verifying all 22nd Assembly members...'))

        # Get all speakers from our database who are marked as 22nd Assembly
        db_speakers_22 = Speaker.objects.filter(
            gtelt_eraco__icontains='22').order_by('naas_nm')

        self.stdout.write(
            f'📊 Found {db_speakers_22.count()} speakers in database marked as 22nd Assembly'
        )
        ...
        if not assembly_api_key:
            self.stdout.write(
                self.style.ERROR(
                    '❌ ASSEMBLY_API_KEY not found in environment variables'))
            return

        self.stdout.write(
            '🌐 Fetching official data from nepjpxkkabqiqpbvk API...')
        ...
        try:
            ...
            if 'nepjpxkkabqiqpbvk' not in data or len(
                    data['nepjpxkkabqiqpbvk']) < 2:
                self.stdout.write(
                    self.style.ERROR('❌ Invalid API response structure'))
                return

            api_members = data['nepjpxkkabqiqpbvk'][1].get('row', [])
            self.stdout.write(
                f'🌐 Fetched {len(api_members)} members from official API')

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Error fetching API data: {e}'))
            return
        ...
        for party, count in sorted(api_party_stats.items(),
                                   key=lambda x: x[1],
                                   reverse=True):
            if count > 0:
        ...
        if missing_from_db:
            ...
            for code in missing_from_db:
                ...
                self.stdout.write(
                    f'   {member["name"]} ({code}) - {member["party"]}')
        ...
        if missing_from_api:
            ...
            for code in missing_from_api:
                ...
                self.stdout.write(
                    f'   {speaker.naas_nm} ({code}) - {speaker.plpt_nm}')
        ...
        if party_mismatches:
            ...
            if len(party_mismatches) > 20:
                self.stdout.write(
                    f'   ... and {len(party_mismatches) - 20} more')
        ...
        problematic_parties = [
            '대한독립촉성국민회', '한나라당', '민주자유당', '정보없음', '민주정의당', '신민당', '바른정당', '한국당'
        ]
        ...
        for party in problematic_parties:
            ...
            if count > 0:
                self.stdout.write(
                    f'   ❌ {party}: {count} members still in database')
        ...
        if update_mode:
            ...
            for mismatch in party_mismatches:
                ...
                if updated_count <= 10:  # Show first 10 updates
                    self.stdout.write(
                        f'   ✅ Updated {speaker.naas_nm}: {api_party}')
        ...
        for party, count in sorted(db_party_stats.items(),
                                   key=lambda x: x[1],
                                   reverse=True):
            if count > 0:
