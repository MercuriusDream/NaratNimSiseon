...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action='store_true',
            help='Create Party records for parties that have members but no Party record',
        )
        parser.add_argument(
            '--show-stats',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔄 Synchronizing party and member relationships...')
        )
        ...
        self.stdout.write(f'📊 Found {len(speaker_parties)} unique parties in speaker data')
        ...
        for speaker_party in speaker_parties:
            ...
            if party_name:  # Skip empty party names
                party, created = Party.objects.get_or_create(
                    name=party_name,
                    defaults={
                        'description': f'정당 - {member_count}명의 의원'
                    }
                )
        ...
        if create_missing:
            self.stdout.write(
                self.style.SUCCESS(f'🎉 Created {created_parties} new party records')
            )
        ...
        if show_stats:
            ...
            for party in Party.objects.all():
                ...
                if member_count > 0:
                    ...
                    self.stdout.write(f'   • Male: {male_count}, Female: {female_count}')
                    ...
                    if committees:
                        ...
                        for committee in committees:
                            if committee['cmit_nm']:
                                self.stdout.write(f'     - {committee["cmit_nm"]}: {committee["count"]} members')
