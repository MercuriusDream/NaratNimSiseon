...
class DataCollector:
    ...
    def fetch_sessions(self, num_records=100, force=False):
        ...
        try:
            ...
            data = response.json()
            if 'nwvrqwxyaytdsfvhu' in data and data['nwvrqwxyaytdsfvhu'][0]['head'][0]['RESULT']['CODE'] == 'INFO-000':
                sessions_data = data['nwvrqwxyaytdsfvhu'][1]['row']
                logger.info(f"Successfully fetched {len(sessions_data)} sessions")
    ...
    def fetch_bills(self, num_records=100, session_id=None):
        ...
        try:
            ...
            data = response.json()
            if 'ppzbyfwhwaoanttzje' in data and data['ppzbyfwhwaoanttzje'][0]['head'][0]['RESULT']['CODE'] == 'INFO-000':
                bills_data = data['ppzbyfwhwaoanttzje'][1]['row']
    ...
    def save_session_data(self, sessions_data, force=False):
        ...
        for session_data in sessions_data:
            try:
                ...
                if not force and Session.objects.filter(sess_id=session_id).exists():
                    continue
                ...
                session, created = Session.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'conf_dt': self._parse_date(session_data.get('CONF_DT')),
                ...
                logger.error(f"Error saving session {session_data.get('SESS_ID')}: {e}")
                continue
            ...
            try:
                ...
                bill, created = Bill.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'propose_dt': self._parse_date(bill_data.get('PROPOSE_DT')),
                ...
                logger.error(f"Error saving bill {bill_data.get('BILL_ID')}: {e}")
                continue
