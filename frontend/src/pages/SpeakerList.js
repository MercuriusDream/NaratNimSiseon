...
with
<new_str>
const response = await api.get(ENDPOINTS.SPEAKERS, {
</new_str>
Fix speakers API endpoint
Replacing
<old_str>
...
The provided changes are meant to replace the API endpoint call with a constant `ENDPOINTS.SPEAKERS`. The original code has:
...
```javascript
const response = await api.get(ENDPOINTS.SPEAKERS + `/?${params}`);
```
